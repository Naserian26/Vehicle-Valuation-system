...
/**
 * Check if user can create user with target role
 */
export const canCreateUser = (currentUserRole, targetRole) => {
  const adminLevel = getRoleLevel(currentUserRole);
  const targetLevel = getRoleLevel(targetRole);

  // Rule 1: Viewers cannot create users
  if (adminLevel <= 1) {
    return { canCreate: false, message: "Viewers cannot create users" };
  }

  // Rule 2: Business Admin cannot create Super Admins
  if (adminLevel === 2 && targetLevel >= 3) {
    return { canCreate: false, message: "Business Admins cannot create Super Admins" };
  }

  // Rule 3: Super Admin cannot create other Super Admins
  if (adminLevel >= 3 && targetLevel >= 3) {
    return { canCreate: false, message: "Super Admins cannot create other Super Admins" };
  }

  return { canCreate: true, message: "User creation allowed" };
};
...
export const getRoleOptions = (currentUserRole, targetUserRole = null) => {
  ...
  if (currentLevel >= 3) { // Super Admin
    ...
    const canModify = targetLevel < 2; // Can only modify Viewers or lower
    options = [
      { value: 'viewer', label: 'Viewer', enabled: canModify },
      { value: 'business_admin', label: 'Business Admin', enabled: canModify }
    ];
  } else { // Viewer
    options = [
...
/**
 * Get available role options for user creation
 */
export const getCreateUserOptions = (currentUserRole) => {
  const currentLevel = getRoleLevel(currentUserRole);
...
export const getUserPermissions = (userRole, matrixData = null) => {
  if (!matrixData) {
    matrixData = DEFAULT_PERMISSIONS;
  }
...
/**
 * Validate vehicle action based on RBAC
 */
export const validateVehicleAction = (userRole, action, vehicleYear = null) => {
  const permissions = getUserPermissions(userRole);

  // Vehicle addition restricted to 2025
  if (action === 'add_vehicles' && vehicleYear !== 2025) {
    return { canPerform: false, message: "Vehicle addition is restricted to year 2025 only" };
  }

  // Check permission for the action
  if (permissions[action]) {
    return { canPerform: true, message: `Permission granted for ${action}` };
  } else {
    return { canPerform: false, message: `Permission denied for ${action}` };
  }
};
